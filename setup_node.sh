...
rhn=`ssh -n -o StrictHostKeyChecking=no -i $kname $ipaddr "hostname"`
ssh -n -o StrictHostKeyChecking=no -i $kname $ipaddr sed -i "/$rhn/d" /etc/hosts
...
do
  ...
  sip=`echo "$i" | cut -d " " -f1`
  shn=`echo "$i" | cut -d " " -f3`
  chk=`ssh -n -o StrictHostKeyChecking=no -i $kname $ipaddr "grep $sip /etc/hosts"`
  if [ -z "$chk" ]; then
    echo `date` setup_node.sh adding master line $i
	ssh -n -o StrictHostKeyChecking=no -i $kname $ipaddr "echo $sip $shn.$DOMAIN $shn >> /etc/hosts"
  fi
...
ssh -o StrictHostKeyChecking=no -i $kname $ipaddr "apt-get update && apt-get install -y build-essential openssh-server openssh-client ntp curl unzip" 

scp -o StrictHostKeyChecking=no -i $kname globals $ipaddr:globals
scp -o StrictHostKeyChecking=no -i $kname $MASTERNODE_FILENAME $ipaddr:$MASTERNODE_FILENAME
scp -o StrictHostKeyChecking=no -i $kname $SLAVENODE_FILENAME $ipaddr:$SLAVENODE_FILENAME
scp -o StrictHostKeyChecking=no -i $kname $MASTERNODE_FILENAME_HOSTS $ipaddr:$MASTERNODE_FILENAME_HOSTS
scp -o StrictHostKeyChecking=no -i $kname $SLAVENODE_FILENAME_HOSTS $ipaddr:$SLAVENODE_FILENAME_HOSTS
